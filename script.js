...
function drawBoard() {
  ...
  drawStones(cellSize);
  drawLastMoveCursor(cellSize);
}

function drawLastMoveCursor(cellSize) {
  if (!lastMove) {
    return;
  }
  ...
  const outerRadius = cellSize * 0.22;
  const innerRadius = cellSize * 0.08;

  ctx.beginPath();
  ctx.arc(x, y, outerRadius, 0, Math.PI * 2);
  ctx.lineWidth = 2;
  ctx.strokeStyle = player === HUMAN_PLAYER ? "#fef3c7" : "#111827";
  ctx.stroke();

  ctx.beginPath();
  ctx.arc(x, y, innerRadius, 0, Math.PI * 2);
  ctx.fillStyle = player === HUMAN_PLAYER ? "#fef3c7" : "#111827";
  ctx.fill();
}
